        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                <article className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <p className="text-xs font-semibold tracking-[0.15em] text-slate-500 uppercase">Quick Insight</p>
                    <p className="mt-2 text-sm font-medium text-slate-700">This panel can host your module-specific KPIs and reports.</p>
                </article>
                <article className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <p className="text-xs font-semibold tracking-[0.15em] text-slate-500 uppercase">Actions</p>
                    <p className="mt-2 text-sm font-medium text-slate-700">Add forms, filters, and action buttons here as your module grows.</p>
                </article>
                <article className="rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:col-span-2 xl:col-span-1">
                    <p className="text-xs font-semibold tracking-[0.15em] text-slate-500 uppercase">Notes</p>
                    <p className="mt-2 text-sm font-medium text-slate-700">Responsive layout is already ready for desktop, tablet, and mobile.</p>
                </article>
            </div>
        </section>
    );
...
const catalogFieldConfig = [
    ...
    { key: 'minimum_order_quantity', label: 'Minimum Order Quantity', type: 'number', min: '1' },
    { key: 'starting_price', label: 'Starting Price', type: 'number', min: '0' },
...
function CatalogInput({ id, label, ...props }) {
    return (
        <div>
            <label htmlFor={id} className="text-xs font-bold tracking-[0.14em] text-slate-500 uppercase">
                {label}
            </label>
            <input
                id={id}
...
export default function Dashboard({ catalogItems = [] }) {
    ...
    const filteredActivityRows = useMemo(() => {
        ...
        return activityRows.filter((row) => {
            const value = `${row.id} ${row.name} ${row.action} ${row.status}`.toLowerCase();
    ...
    const filteredCatalogItems = useMemo(() => {
        ...
        return catalogItems.filter((item) => {
            const value = `${item.name} ${item.category} ${item.print_type} ${item.starting_price}`.toLowerCase();
            return value.includes(query);
        });
    }, [catalogItems, searchTerm]);

    const editingCatalogItem = useMemo(() => {
        return catalogItems.find((item) => item.id === editingCatalogId) ?? null;
    ...
    const handleCatalogUpdate = (event) => {
        ...
        editCatalogForm.put(route('dashboard.catalog-items.update', editingCatalogId), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleCatalogDelete = (catalogItemId) => {
        ...
        deleteCatalogForm.delete(route('dashboard.catalog-items.destroy', catalogItemId), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const renderCatalogForm = () => (
        <section className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm">
            <h3 className="text-lg font-black text-slate-900">Add Catalog Item</h3>
            <p className="mt-1 text-sm text-slate-500">Create a new catalog product using the fields below.</p>
                    ...
                    {catalogFieldConfig.map((field) => (
                        <div key={field.key}>
                            <CatalogInput
                                ...
                                onChange={(event) => catalogForm.setData(field.key, event.target.value)}
                            />
                            <CatalogInputError message={catalogForm.errors[field.key]} />
                        </div>
                    ))}
        ...
        <section className="space-y-6">
            <section className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm">
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <h3 className="text-lg font-black text-slate-900">Catalog Items List</h3>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                        ...
                        >
                            ...
                            <div className="p-4">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${item.is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>
                                        {item.is_active ? 'Active' : 'Inactive'}
                                    </span>
                                </div>
                                <div className="mt-3 flex items-center justify-between text-xs font-semibold text-slate-600">
                                    <span>MOQ {item.minimum_order_quantity}</span>
                                    <span>PKR {item.starting_price}</span>
                                </div>
                                <div className="mt-4 flex items-center gap-2">
                                    <button
                                        type="button"
                                        onClick={() => handleCatalogEditStart(item)}
                    ...
                    {filteredCatalogItems.length === 0 && (
                        <article className="sm:col-span-2 xl:col-span-3 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-8 text-center">
                            <p className="text-sm font-semibold text-slate-600">No catalog items found.</p>
                        </article>
                    )}
                </div>
            </section>
            ...
            {editingCatalogItem && (
                <section className="rounded-3xl border border-slate-200 bg-white/95 p-6 shadow-sm">
                    ...
                    <form onSubmit={handleCatalogUpdate} className="mt-5 space-y-4">
                        <div className="grid gap-4 md:grid-cols-2">
                            {catalogFieldConfig.map((field) => (
                                <div key={`edit-${field.key}`}>
                                    <CatalogInput
                                        ...
                                        onChange={(event) => editCatalogForm.setData(field.key, event.target.value)}
                                    />
                                    <CatalogInputError message={editCatalogForm.errors[field.key]} />
                                </div>
                            ))}
                            <div className="flex items-end pb-1">
                                <label className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-slate-700">
                                    <input
                                        ...
                                        onChange={(event) => editCatalogForm.setData('is_active', event.target.checked)}
                                        className="rounded border-slate-300 text-cyan-600 focus:ring-cyan-500"
                                    />
                                    Active Item
                                </label>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                {editCatalogForm.processing ? 'Updating...' : 'Save Update'}
                            </button>
                            <button
                                type="button"
                                onClick={() => {
            ...
            <div className="min-h-screen bg-gradient-to-b from-slate-100 via-cyan-50/50 to-slate-100 text-slate-800">
                ...
                <div className="mx-auto flex w-full max-w-[1850px] gap-6 px-4 py-6 sm:px-6 lg:px-8">
                    <DashboardSidebar
                        ...
                        onCatalogToggle={() => setIsCatalogOpen((value) => !value)}
                    ...
                    <main className="min-w-0 flex-1 space-y-6">
                        ...
                        {activeModule === 'dashboard' && (
                            <>
                                <section className="rounded-3xl bg-gradient-to-r from-slate-900 via-cyan-900 to-sky-900 p-6 text-white shadow-lg sm:p-8">
                                    <p className="text-xs font-semibold tracking-[0.2em] uppercase text-cyan-200">Modern Dashboard</p>
                                    <h1 className="mt-3 text-3xl font-black leading-tight sm:text-4xl">Business Analytics Overview</h1>
                                    <p className="mt-3 max-w-2xl text-sm leading-7 text-cyan-100">
                                        A clean responsive dashboard with metrics, charts, activities, and a live preview panel.
                                    </p>
                                </section>
                        ...
                        {activeModule === 'catalog' && (
                            <>
                                <section className="rounded-3xl bg-gradient-to-r from-slate-900 via-cyan-900 to-sky-900 p-6 text-white shadow-lg sm:p-8">
                                    <p className="text-xs font-semibold tracking-[0.2em] uppercase text-cyan-200">Catalog Module</p>
                                    <h2 className="mt-3 text-3xl font-black leading-tight sm:text-4xl">Manage Catalog Items</h2>
                                    <p className="mt-3 max-w-2xl text-sm leading-7 text-cyan-100">
                                        Use the dropdown tabs in sidebar to quickly switch between adding and viewing catalog items.
                                    </p>
                                </section>

                                {catalogTab === 'add' ? renderCatalogForm() : renderCatalogList()}
                            </>
                        )}
